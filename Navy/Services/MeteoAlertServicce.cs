...
namespace Navy.Services
{
    ...
    {       internal static string GetTiempo()
        {
            var options = new RestClientOptions("https://www.el-tiempo.net/")
            {
                MaxTimeout = -1,
            };
            var client = new RestClient(options);
            var request = new RestRequest("api/json/v2/provincias/28/municipios/28007", Method.Get);
            RestResponse response = client.Execute(request);
            var meteo = JsonConvert.DeserializeObject<Root>(response.Content);
            if (!string.IsNullOrEmpty(meteo.lluvia))
            {
                ...
                meteo.lluvia = total != 0 ? total.ToString(CultureInfo.InvariantCulture) : "";
            }
            return meteo.lluvia;
        }
    }
